...
const AdminPage: NextPage = () => {
  ...
  const [selectedUser, setSelectedUser] = useState<User | null>(null) // Add state to keep track of selected user
  ...
  const handleLoginClick = () => {
    if (
      ...
      hideLoging() // Hide login form and show user edit form
    } else {
  ...
  const handleEditSubmit = async () => {
    try {
      const { _id, ...rest } = selectedUser as User // Destructure selectedUser to get only the updated values
      ...
      setSelectedUser(null) // Reset selected user after successful edit
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
          user._id === _id ? { ...user, ...rest } : user
        )
      ) // Update edited user in state
    } catch (error) {
    ...
    <div>
      ...
      {showForm &&
        ...
        users.map((user) => {
          ...
          const handleEditSubmit = async () => {
            try {
              const { id, ...userToUpdate } = editValues
  
              const response = await fetch(
                `http://localhost:3000/api/editUser?id=${id}`,
                {
                  method: "PATCH",
                  headers: {
                    "Content-Type": "application/json",
...
//                         value={editValues.post}
//                         onChange={(e) =>
//                           setEditValues({ ...editValues, post: e.target.value })
//                         }
//                       ></textarea>
//                     </td>
//                   </tr>
